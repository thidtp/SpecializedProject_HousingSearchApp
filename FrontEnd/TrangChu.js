...
window.scrollTo({ top: 0, behavior: 'smooth' });});
    ...
    openRightMenuButton.addEventListener('click', () => { 
      ...
      const headerContainer = document.querySelector('.nav2-container');
      let isHeaderSticky = false;
  
      window.addEventListener('scroll', () => {
          console.log('Scroll Y:', window.scrollY);
          if (window.scrollY > 0 && !isHeaderSticky) {
              console.log('Sticky Nav  Activated');
              headerContainer.classList.remove('bg-transparent', 'text-white');
              headerContainer.classList.add('bg-gray-300', 'text-black','shadow-light');
              isHeaderSticky = true;
          } else if (window.scrollY === 0 && isHeaderSticky) {
              console.log('Sticky Header Deactivated');
              headerContainer.classList.remove('bg-gray-300', 'text-black');
